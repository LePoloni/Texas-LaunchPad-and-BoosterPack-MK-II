...
#include <stdint.h>
#include <math.h>	//Leandro (19/03/2019)
#include "BSP.h"
...
//Leandro (19/03/201)
//------------BSP_LCD_Rect------------
// Draw a rectangle at the given coordinates with the given width, height, and color.
// Requires (4*11 + 2*h + 2*w) bytes of transmission (assuming image fully on screen)
// Input: x     horizontal position of the top left corner of the rectangle, columns from the left edge
//        y     vertical position of the top left corner of the rectangle, rows from the top edge
//        w     horizontal width of the rectangle
//        h     vertical height of the rectangle
...
void BSP_LCD_Rect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color) 
{
  ...
  if((y + h - 1) >= _height) h = _height - y;
	...
	BSP_LCD_DrawFastHLine(x,   y,   w, color);
	BSP_LCD_DrawFastHLine(x,   y+h, w, color);
	BSP_LCD_DrawFastVLine(x,   y,   h, color);
	BSP_LCD_DrawFastVLine(x+w, y,   h, color);
}
